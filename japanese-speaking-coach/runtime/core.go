...
func configPath() string {
	return filepath.Join(codexHome(), "japanese-speaking-coach", "workspace.json")
}
func defaultRoot() string { return filepath.Join(filepath.Dir(configPath()), "data") }
func skillRoot() string {
	if p := os.Getenv("JAPANESE_COACH_SKILL_ROOT"); p != "" {
		return absolute(p)
	}
	exe, e := os.Executable()
	must(e)
...
func workspace(root string) (result M) {
	defer func() {
		if p := filepath.Join(str(result["data_root"]), "profile.json"); exists(p) {
