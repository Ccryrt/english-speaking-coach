...
func coalesceExpressions(d M) M {
	...
	for i, v := range arr(d["expressions"]) {
		raw := obj(v)
		key := canonicalKey(str(raw["japanese"]), str(raw["chinese"]))
...
func reconcileReview(d M, language string) M {
	...
	for i, v := range arr(d["expressions"]) {
		...
		en := strings.TrimSpace(str(x["japanese"]))
		if err := attempt(func() { validateGuide(g, en) }); err != nil {
			delete(x, "reading_guide")
			note := "Reading annotations did not pass validation and are withheld; the Japanese expression is retained."
			if language == "zh-CN" {
				note = "读法标注未通过校验，暂不展示；日文表达已保留。"
			}
...
func normalizeReview(root string, d M, thread, voice string, snapshot, state M) M {
	...
	for _, v := range arr(state["expressions"]) {
		...
		used[str(e["id"])] = true
		byText[canonicalKey(str(e["japanese"]), str(e["chinese"]))] = e
	}
	...
	for _, v := range arr(d["expressions"]) {
		...
		if ref != "" {
			...
			for _, k := range []string{"japanese", "chinese"} {
			...
			require(validExpressionLanguages(x), "表达需要日文原句与中文句意；未把错位语言保存为学习资料。")
			ref = str(obj(byText[canonicalKey(str(x["japanese"]), str(x["chinese"]))])["id"])
		}
		if ref == "" {
		...
		byText[canonicalKey(str(x["japanese"]), str(x["chinese"]))] = x
	}
	concepts := M{}
...
func compactReviewInput(state, snapshot M) M {
	...
	for _, v := range reverse(arr(state["expressions"])) {
		e := obj(v)
		if strings.Contains(text, strings.ToLower(str(e["japanese"]))) {
			catalog = append(catalog, pick(e, "id", "japanese", "chinese"))
		}
	}
	for _, v := range arr(state["concepts"]) {
		c := obj(v)
...
func checkedPreview(expressions A, snapshot M) A {
	...
	for _, v := range head(expressions, 3) {
		x := obj(v)
		good := true
		for _, k := range []string{"original", "japanese", "chinese"} {
		...
		if good && linked && validExpressionLanguages(x) {
			out = append(out, pick(x, "original", "japanese", "chinese", "note", "source_turn_ids"))
		}
	}
	return out
}
