	...
	for _, v := range rows {
		...
		if len(prior) > 0 {
			...
			if merged["text"] != old["text"] {
				sqlExec(tx, "UPDATE segments SET text=?,chinese=NULL,reading=NULL,status='pending',attempts=0,translated_at=NULL,latency=NULL WHERE run=? AND id=?", r["text"], run, r["id"])
...
func (l *Live) translated(run string, rows A, latency float64, expected M) {
	for _, v := range rows {
		r := obj(v)
		sqlExec(l.db, "UPDATE segments SET chinese=?,reading=?,status='translated',translated_at=?,latency=? WHERE run=? AND id=? AND text=? AND (status!='translated' OR chinese IS NOT ? OR reading IS NOT ?)", r["chinese"], r["reading"], now(), latency, run, r["id"], expected[str(r["id"])], r["chinese"], r["reading"])
	}
}
...
func (l *Live) rejectTranslations(run string, rejected, expected M) {
	...
	for id := range rejected {
		sqlExec(l.db, "UPDATE segments SET chinese=NULL,reading=NULL,translated_at=NULL,latency=NULL,status=CASE WHEN attempts>=2 THEN 'failed' ELSE 'pending' END WHERE run=? AND id=? AND text=?", run, id, expected[id])
	}
}
