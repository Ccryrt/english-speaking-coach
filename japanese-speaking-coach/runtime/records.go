...
func profileDefault() M {
	return M{"schema_version": 1, "target_language": "ja", "goal": "在日本的日常生活、工作沟通与面试中清楚自然地表达", "practice_language": "japanese_first", "topic": "mixed", "help_language": "zh-CN", "mode": "roleplay", "correction": "in_character", "drills": "guided", "review_delivery": "written", "review_limit": 2, "input_support": "adaptive", "source_ids": A{}, "updated": today()}
}
func initialize(root string) {
	...
	if !exists(p) {
		old := obj(maybeJSON(filepath.Join(root, "state.json"), M{"schema_version": 1, "project_id": "PRJ-JAPANESE-SPEAKING", "updated": "", "learner": M{}, "sessions": A{}, "expressions": A{}, "weekly_reviews": A{}}))
...
func validateProfile(p M) {
	require(p["target_language"] == "ja", "这不是日语学习档案；请使用独立日语目录")
	require(p["practice_language"] == "japanese_first" || p["practice_language"] == "bilingual", "这不是日语学习档案；请使用独立日语目录")
	require(strings.TrimSpace(str(p["goal"])) != "", "goal must be nonempty")
	choices := map[string][]string{"practice_language": {"japanese_first", "bilingual"}, "help_language": {"zh-CN", "ja"}, "topic": {"mixed", "daily", "work", "interview"}, "mode": {"conversation", "roleplay", "focused"}, "correction": {"light", "detailed", "after_scene", "in_character"}, "drills": {"on_request", "guided"}}
...
func validatePayload(d M, inProgress bool) {
	...
	for _, v := range arr(d["expressions"]) {
		x := obj(v)
		for _, k := range []string{"id", "japanese", "chinese", "mastery", "next_review", "note"} {
			require(strings.TrimSpace(str(x[k])) != "", "Expression missing text: "+k)
		}
		require(validExpressionLanguages(x), "表达必须包含日语和中文释义")
		validateJapaneseNotes(x)
		id := str(x["id"])
		...
		if x["reading_guide"] != nil {
			validateGuide(obj(x["reading_guide"]), str(x["japanese"]))
		}
...
func recordFrontmatter(d M) string {
	m := M{"id": d["id"], "type": "japanese-practice", "sensitivity": "private", "created": d["date"], "source_ids": d["source_ids"]}
...
func sessionMarkdown(d M) string {
	...
	for _, v := range arr(d["expressions"]) {
		...
		if reading := str(x["reading"]); reading != "" {
			out += "假名读音：" + reading + "\n\n"
		}
		if register := str(x["register"]); register != "" {
			out += "适用对象与礼貌：" + register + "\n\n"
		}
		if x["reading_guide"] != nil {
...
func buildState(root string) M {
	...
	for _, v := range records {
		...
		for _, v := range arr(r["expressions"]) {
			...
			if newer {
				merged = copyM(prior)
			} else if merged["reading_guide"] == nil && prior["japanese"] == x["japanese"] && prior["reading_guide"] != nil {
...
func rebuild(root string) M {
	...
	index := "# 日语学习手记 / Japanese practice journal\n\n[本地阅读页](dashboard.html)\n\n"
	htmlText := "<!doctype html><html lang=zh-CN><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><title>日语学习手记</title><style>body{font:18px system-ui;max-width:850px;margin:3rem auto;padding:1rem;line-height:1.7}article{border-bottom:1px solid #ddd;padding:1rem 0}blockquote{margin:1rem 0;padding:1rem;background:#f3f6fa;border-radius:10px}small{color:#556}a{color:#245bb7}</style><h1>日语学习手记</h1><p>离线阅读快照 / Offline reading snapshot</p>"
	for _, v := range reverse(arr(s["sessions"])) {
		...
		for _, v := range arr(record["expressions"]) {
			e := obj(v)
			htmlText += "<blockquote><strong>" + html.EscapeString(str(e["japanese"])) + "</strong><br>" + html.EscapeString(str(e["chinese"])) + "<br><small>原话 / Original: " + html.EscapeString(str(e["original"])) + "</small><p>" + html.EscapeString(str(e["note"])) + "</p></blockquote>"
		}
		if supplement != "" {
