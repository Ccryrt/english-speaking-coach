...
func codexExecutable() string {
	candidates := []string{os.Getenv("JAPANESE_COACH_CODEX"), "/Applications/Codex.app/Contents/Resources/codex", "/Applications/ChatGPT.app/Contents/Resources/codex"}
...
func (c *ModelClient) connect(ctx context.Context) {
	c.close()
	dir, e := os.MkdirTemp("", "japanese-coach-model-")
	must(e)
	c.scratch = dir
	...
	c.request(ctx, "initialize", M{"clientInfo": M{"name": "japanese_coach_companion", "version": version}, "capabilities": M{"experimentalApi": true}})
...
var kanaRE = regexp.MustCompile(`[\p{Hiragana}\p{Katakana}ー]`)
var latinRE = regexp.MustCompile(`[\p{Latin}\p{M}]+`)

// Japanese kanji and Chinese share characters. Translate each complete turn with
// context instead of guessing language from its script or splitting Latin words.
func translationUnits(segments A) A {
	units := A{}
	for _, v := range segments {
		s := obj(v)
		text := str(s["text"])
		require(strings.TrimSpace(text) != "" && len([]rune(text)) <= 12000, "转写为空或过长；原话已保留。")
		units = append(units, M{"id": str(s["id"]) + "/japanese/0", "segment_id": s["id"], "text": text, "start": 0, "end": len(text)})
	}
	return units
}
...
func assembleTranslations(segments, units, translated A) A {
	...
	require(len(byID) == len(units), "日文片段未全部获得译文；原话已保留。")
	for _, v := range units {
		u := obj(v)
		t := obj(byID[str(u["id"])])
		zh := str(t["chinese"])
		...
		if t["kind"] == "name" {
			...
			require(t["kind"] == "translation" && hanRE.MatchString(zh) && !kanaRE.MatchString(zh), "译文未给出中文句意，或仍含日语假名。")
			if len(ws) >= 2 {
				require(!strings.Contains(normalizeLatin(zh), normalizeLatin(str(u["text"]))), "译文只是日文回声。")
			}
		}
	}
	out := A{}
	for _, v := range segments {
		...
		row := M{"id": s["id"], "chinese": strings.TrimSpace(zh)}
		for _, v := range units {
			u := obj(v)
			reading := str(obj(byID[str(u["id"])])["reading"])
			if u["segment_id"] == s["id"] && validKanaReading(reading) {
				row["reading"] = reading
			}
		}
		out = append(out, row)
...
func suggestionPrefix(text string) A {
	...
	for len(items) < 3 && strings.HasPrefix(rest, "{") {
		...
		if fields["source_turn_ids"] != nil && fields["original"] != nil && fields["japanese"] != nil && fields["chinese"] != nil {
			items = append(items, pick(fields, "source_turn_ids", "original", "japanese", "chinese"))
		}
