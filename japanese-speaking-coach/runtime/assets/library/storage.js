(() => {
  ...
  function shell(data) {
    return `<section class="panel storage-transfer"><h2>备份、换电脑与更换目录</h2>
      ...
      <input id="storage-destination" name="destination" type="text" required autocomplete="off" placeholder="例如 /Users/你的用户名/Documents/Japanese-Learning">
  ...
  function mount(data) {
    ...
    q('#download-backup').addEventListener('click',e=>run(e.currentTarget,async()=>{
      ...
      a.href=url;a.download='japanese-learning-'+new Date().toISOString().slice(0,10)+'.zip';
