...
window.CoachLive = (() => {
  ...
  function utterance(x, open, state) {
    ...
    const annotation=tail?`<p class="live-fragment">转写续接 · 可能与前段连读 <strong lang="ja">${e(fragment.joined_word)}</strong>，不是单独词条。</p>`:fragment?'<p class="live-fragment">前句续接 · 原始转写片段</p>':'';
    ...
    const reading=x.reading?`<details class="live-answer" data-segment="reading-${e(x.id)}" ${open.has('reading-'+x.id)?'open':''}><summary>假名读音</summary><p lang="ja">${e(x.reading)}</p><p class="fine">参考读法</p></details>`:'';
    return `<article data-row="${e(x.id)}" class="live-utterance ${x.role==='user'?'live-user':'live-coach'}"><div class="live-speaker"><span>${x.role==='user'?'你 · YOU':'教练 · COACH'}</span><time>${e(stamp(x.timestamp))}</time></div><p class="live-japanese" lang="ja">${e(x.text)}</p>${annotation}${translation}${reading}</article>`;
  }
  function paint(data) {
    current = data;
    const s=data.state, feed=get('#live-feed');if(!feed)return;
    if(get('#live-scene'))get('#live-scene').textContent=s?.scene_introduction||'继续用日文聊。没听懂时，看一眼这里。';
    ...
    const runOptions=[`<option value="">当前伴随</option>`,...data.history.map(r=>`<option value="${e(r.id)}">${e(new Date(r.voice_started_at||r.created_at).toLocaleString('zh-CN'))} · ${r.demo?'测试演示':r.recovery_mode==='after_voice'?'结束后恢复':'日语练习'}</option>`)].join('');
    ...
    const signature=JSON.stringify([s?.id,s?.voice_id,s?.status,s?.translation_status,s?.ready,data.items.map(x=>[x.seq,x.text,x.status,x.chinese,x.reading,x.fragment]),chinese,options.page,options.offset]);
