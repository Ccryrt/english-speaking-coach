...
const bookJapanese = {'日常生活':'暮らし','工作沟通':'仕事','求职面试':'面接'};
...
function japaneseNotes(e) {
  const kinds={error:'纠错',naturalness:'更自然的说法',register:'场合与礼貌',already_correct:'可以继续使用'};
  return (e.reading?`<details class="reading-guide"><summary>假名读音</summary><p lang="ja">${esc(e.reading)}</p><p class="fine">参考读法，不是发音评分。</p></details>`:'') + (e.register?`<p class="fine">适用对象与礼貌：${esc(e.register)}</p>`:'') + (kinds[e.correction_kind]?`<p class="fine">${esc(kinds[e.correction_kind])}</p>`:'');
}
function readingGuide(e) {
  ...
  return `<details class="reading-guide"><summary>怎么念 · 怎么记 <span>日语表达块</span></summary><div class="reading-body"><p class="fine">按意思分组帮助记忆，不要求每组停顿。假名是参考读法，不是音调判定。</p><p class="reading-line" lang="ja">${g.groups.map(group=>`<span class="reading-group">${marked(group)}</span>`).join('<span class="reading-break" aria-label="可轻停"> / </span>')} <span class="reading-tone" aria-hidden="true">${arrows[g.tone]||''}</span></p><p class="tone-note">${esc(g.tone_note)}</p><h3>先记住这些表达块</h3><div class="memory-parts">${g.memory.map(p=>`<div><strong lang="ja">${esc(p.text)}</strong><span>${esc(p.meaning)}</span></div>`).join('')}</div><p class="fine">记忆块方便起头和替换，不要求每块都停顿。这是参考读法，不是实际语音评分。</p></div></details>`;
}
function attemptEvidence(e) {
  const a=e.attempt_evidence;
  if(!a?.learner?.quote)return '';
  return `<details class="source-quotes"><summary>这次尝试的依据</summary>${a.coach?.quote?`<span class="small-label">教练先给出的提示</span><p lang="ja">${esc(a.coach.quote)}</p>`:''}<span class="small-label">我随后说</span><p lang="ja">${esc(a.learner.quote)}</p><p class="fine">${a.coach?'有提示的尝试，不代表独立掌握。':'此条记录引用了学习者的实际发言。'}</p></details>`;
}
function excerpt(e) {
  return `<article class="excerpt"><span class="small-label">我当时说</span><p class="original" lang="ja">${esc(e.original || '这条没有记录原话。')}</p><span class="small-label">${e.original?.trim()===e.japanese.trim()?'这句话可以继续用':'表达参考'}</span><p class="model" lang="ja">${esc(e.japanese)}</p><p class="chinese">${esc(e.chinese)}</p><p class="evidence-note">${esc(e.note)}</p>${e.source_quotes?.length>1?`<details class="source-quotes"><summary>这句话的求助与后续尝试</summary>${e.source_quotes.map(q=>`<p>${esc(q.quote)}</p>`).join('')}</details>`:''}${japaneseNotes(e)}${attemptEvidence(e)}${readingGuide(e)}</article>`;
}
...
function termsPage(data, args) {
  ...
  return heading('WORDS & EXPRESSIONS','生词与表达','按课次回顾，或查看所有积累。') +
    ...
    `<div class="study-toolbar"><div class="study-modes" role="group" aria-label="查看方式"><button type="button" data-study-mode="speak" aria-pressed="${mode==='speak'}">练表达<span>中文 → 日文</span></button><button type="button" data-study-mode="meaning" aria-pressed="${mode==='meaning'}">认词义<span>日文 → 中文</span></button><button type="button" data-study-mode="read" aria-pressed="${mode==='read'}">日中对照<span>一起查看</span></button></div><p>${mode==='speak'?'先自己说一句，再点击卡片翻到日文答案。':mode==='meaning'?'先想一想词义，点击卡片翻到中文答案。':'中日文同时显示，方便查找和阅读。'}</p></div><div class="filter-meta"><span>${args.session?'来自所选对话 · ':''}${args.due==='1'?'建议回顾 · ':''}筛选后 ${data.total} 张 · 本页显示 ${data.items.length} 张 ${tip('表达状态','反映最近有证据的一次提示情况，不表示永久掌握或日语等级。查看详情可追溯原话和复述记录。自己翻卡不会改变学习状态。')}</span>${recall&&data.items.length?'<button type="button" class="button small" id="hide-answers">隐藏本页全部答案</button>':''}</div>`+
...
function progressRow(c,args={},summary=false) {
  return `<article class="progress-row"><div><h3><a href="${esc(progressLink(c,args))}" lang="ja">${esc(c.term)}</a></h3><p>${esc(c.meaning)}</p></div><div><span class="tag ${c.needs_revisit?'neutral':c.level==='stable'?'green':''}">${esc(c.needs_revisit?'建议再练':c.level_label)}</span><p>${esc(c.change_text)}</p></div><div class="progress-row-meta"><span>${esc(c.latest_in_period)} · ${c.period_event_count} 条表现记录</span><a href="${esc(progressLink(c,args))}">查看历程 ↗</a></div></article>`;
}
...
async function showTerm(id) {
  try {
    ...
    $('#dialog-content').innerHTML = `${tag(t.book)}<h2 class="dialog-title" id="dialog-title" lang="ja">${esc(t.japanese)}</h2><p class="muted">${esc(t.chinese)}</p>${speechButton(t.japanese)}${readingGuide(t)}<div class="dialog-section"><h3>我当时说</h3><p>${esc(t.original || '这条没有记录原话。')}</p></div><div class="dialog-section"><h3>用法与练习观察</h3><p>${esc(t.note)}</p><p class="evidence-note">${esc(t.state_label)} · 最近记录于 ${esc(t.updated)}<br>建议再聊：${esc(t.next_review)}。翻看答案不会改变这个状态。</p></div>${t.attempts?.length?`<details class="source-box"><summary>查看 ${t.attempts.length} 条练习观察</summary>${t.attempts.map(a=>`<p>${esc(a.date)} · ${esc(labels[a.prompt]||a.prompt)} · ${esc(a.note||'旧记录未保存更详细的提示过程。')}</p>`).join('')}</details>`:''}<div class="dialog-section"><h3>回到来源对话</h3><div class="source-links">${t.sources.map(s=>`<a href="${esc(href('sessions/'+s.id))}">${esc(shortDate(s.date))} · ${esc(s.title)} ↗</a>`).join('')}</div></div>`;
...
async function render() {
  ...
  try {
    ...
    if(version!==renderVersion)return;
    markup=markup.replace('读法需要示范时，可以在 Voice 中向教练提问。原话保留供对照。<br>','你的原话保留供对照；书面表达建议不冒充会中原话。<br>');
    ...
    paintReviewNotice();
    document.title=(names[section]||'我的学习')+' · 日语学习档案';
...
function reviewPreview(data) {
  ...
  return `<h2>先看这几句</h2><p class="fine">表达建议 · 完整复盘${data.status==='error'?'暂未完成，可重试':'仍在补全'}。这里不代表已经掌握，也不会提前计入学习记录。</p>`+
    data.preview.map(x=>`<article class="review-preview-expression"><p class="fine">当时说：${esc(x.original)}</p><p class="preview-japanese" lang="ja">${esc(x.japanese)}</p>${speechButton(x.japanese)}<p>${esc(x.chinese)}</p></article>`).join('');
}
...
function rememberReview(data,args={}) {
  ...
  if(item.status==='saved' && prior && prior.status!=='saved' && !reviewSeen.has(key)) {
    reviewSeen.set(key,true);toast((item.title||'日语练习')+'的复盘已保存，可从页面上方打开。');
  }
}
...
function reviewLabel(item) {
  ...
  return `${item.title || '本次日语练习'}${time?' · '+time:''}`;
}
