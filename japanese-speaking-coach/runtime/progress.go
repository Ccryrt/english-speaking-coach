...
func validateGuide(g M, japanese string) {
	...
	for _, v := range groups {
		...
		stress := arr(x["stress"])
		require(len(stress) == 0, "Japanese guidance does not mark English-style stressed words")
	...
	require(strings.Join(strings.Fields(strings.Join(all, "")), "") == strings.Join(strings.Fields(japanese), ""), "Reading groups must preserve Japanese text in order")
