...
func checkService(base, root string, expected bool) M {
	...
	require(d["application"] == "japanese-speaking-coach" && absolute(str(d["data_root"])) == absolute(root), "Port belongs to another service/archive; it was preserved")
	if expected {
...
func serviceStart(root string, managed bool) M {
	...
	require(freePort(8898), "固定端口 8898 被其他实例占用；已保留占用者，请让 Agent 核对。")
	var out M
	withLock(filepath.Join(filepath.Dir(configPath()), ".service.lock"), func() {
		...
		require(freePort(8898), "端口已被占用，未替换服务。")
		...
		must(e)
		exe = absolute(exe)
		args := []string{"serve", "--port", "8898", "--codex-home", codexHome(), "--skill-root", skillRoot()}
		binary := os.Getenv("JAPANESE_COACH_CODEX")
		if binary == "" {
		...
		id := "japanese-speaking-coach-" + hash([]byte(configPath()))[:12]
		...
		case "darwin":
			...
			env := "<key>CODEX_HOME</key><string>" + xmlText(codexHome()) + "</string><key>JAPANESE_COACH_SKILL_ROOT</key><string>" + xmlText(skillRoot()) + "</string>"
			if c := os.Getenv("JAPANESE_COACH_CODEX"); c != "" {
				env += "<key>JAPANESE_COACH_CODEX</key><string>" + xmlText(c) + "</string>"
			}
		...
		case "linux":
			unit := id + ".service"
			cmdArgs := []string{"--user", "--unit", unit, "--property=Restart=on-failure", "--property=RestartSec=5s", "--property=StartLimitIntervalSec=60s", "--property=StartLimitBurst=3", "--setenv=CODEX_HOME=" + codexHome(), "--setenv=JAPANESE_COACH_SKILL_ROOT=" + skillRoot(), "--", exe}
