...
func (s *Server) identity() M {
	exe, e := os.Executable()
	must(e)
	return M{"application": "japanese-speaking-coach", "runtime": "go", "version": version, "code_revision": revision, "pid": os.Getpid(), "executable": exe, "data_root": s.archive.root, "skill_root": skillRoot(), "workspace_managed": s.managed, "instance_id": s.instance, "platform": runtime.GOOS + "/" + runtime.GOARCH}
}
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		case "/api/storage/backup":
			...
			w.Header().Set("Content-Disposition", "attachment; filename=japanese-learning-backup.zip")
...
func serve(ctx context.Context, root string, port int, managed bool) {
	...
	fmt.Printf("Japanese learning archive: http://127.0.0.1:%d\n", s.port)
	e = server.Serve(ln)
