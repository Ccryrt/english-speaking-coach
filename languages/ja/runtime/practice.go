...
func chooseScene(root string, context M) M {
	...
	preferred := textOr(profile["topic"], "mixed")
	if preferred == "mixed" {
		preferred = ""
	}
	scenes := A{}
	...
	x := arr(scenes[0])
	intro := fmt.Sprintf("場所は%sです。あなたは%s、私は%sです。目標は%sです。", str(x[1]), str(x[2]), str(x[3]), str(x[4]))
	return M{"setting": x[1], "learner_role": x[2], "partner_role": x[3], "goal": x[4], "introduction": intro, "opening_line": x[5], "category": x[6]}
}
func validateScene(scene M) {
	if category := scene["category"]; category != nil {
		require(has(stringsA("daily", "work", "interview"), category), "Unknown scene category")
	}
	...
	for k := range scene {
		require(has(stringsA(append(fields, "key_terms", "category")...), k), "Unknown scene field")
	}
...
func speakingContext(profile M, companion bool, phase string, scene M) M {
	...
	interview := profile["topic"] == "interview" || scene != nil && scene["category"] == "interview"
	deferred, inCharacter := profile["correction"] == "after_scene" || interview, profile["correction"] == "in_character" && !interview
	prose := speakingTemplates
	language := str(obj(prose["language"])["japanese_first"])
	if companion {
	...
	if roleplay && scene != nil {
		...
		brief += "\nIntroduce this fresh scene once in Japanese; translate any non-Japanese preparation data before speaking: " + intro + "\nThen open in Japanese with an open question, adapting any closed prompt without adding candidate answers: " + str(scene["opening_line"]) + "\n"
		if len(arr(scene["key_terms"])) > 0 {
			brief += "Use scene.key_terms as preparation candidates: preview only a necessary comprehension word in Japanese before the role opening, then let the learner respond. Do not prefill the learner answer, read the whole plan or assume the terms are unknown.\n"
		}
	}
	...
	return M{"phase": phase, "voice_brief": voiceBrief, "scene": displayScene, "startup": M{"scene_required": roleplay, "scene_selected": roleplay && scene != nil, "history_use": "learning_only", "next_action": next}, "policy": M{"role": policyRole, "correction_timing": func() any {
		if interview {
			return "after_answer"
		}
		return profile["correction"]
	}(), "review_delivery": get(profile, "review_delivery", "spoken"), "input_support": get(profile, "input_support", "adaptive"), "history_continuation": false, "response_language": "japanese_first", "non_japanese_input": "chinese_help_when_needed", "question_style": "open_without_supplied_choices", "proactive_teaching": phase == "review" || !(deferred || inCharacter), "missing_expression_help": "immediate_before_content", "japanese_structure_help": structureHelp, "scene_completion": "ask_open_next_step", "next_action": "make_relevant_next_step_visible", "unsolicited_drills": false, "embedded_recasts": phase == "scene" && inCharacter, "learner_expansion": phase == "scene" && inCharacter, "guided_drills": phase == "review" && profile["drills"] == "guided"}}
}

func resumeContext(root, day, phase string, scene M, topic string) M {
	state := buildState(root)
	profile := copyM(state["profile"])
	if topic != "" {
		profile["topic"] = topic
...
func transitionContext(c M, event string) M {
	...
	case "user_end", "host_closed":
		...
		c["phase"] = nil
		c["voice_brief"] = "Stop spoken practice. A brief Japanese goodbye is enough. The Agent still completes the selected written review; stopping speech does not cancel closeout."
		c["closeout"] = M{"required": true, "next_action": "review-begin", "match": "thread_and_voice", "spoken_review": false, "written_review": true}
	case "pause":
		action = "pause"
		move["continue_voice"] = false
		move["spoken_review"] = false
		c["voice_brief"] = "The learner paused. Acknowledge briefly in Japanese and wait; do not start review or another exercise."
...
func preparePractice(args M) M {
	...
	c := resumeContext(root, today(), str(args["phase"]), scene, str(args["topic"]))
	...
	if enabled {
		...
		writeJSON(filepath.Join(root, "Runtime", "ReviewWatches", str(binding["id"])+".json"), M{"thread_id": thread, "source": absolute(source), "run_id": binding["id"], "title": textOr(scene["setting"], "本次日语练习")})
	}
	if !enabled && str(args["thread-id"]) != "" {
		...
		setReviewStage(root, thread, voice, "practicing", M{"source": source, "auto_review": true, "title": textOr(scene["setting"], "本次日语练习")}, false)
	}
	if base == "" {
