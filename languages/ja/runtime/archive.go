...
func bookFor(s M) string {
	...
	for _, pair := range [][2]string{{"面试|面接|志望|转职|転職|自己紹介", "求职面试"}, {"工作|职场|職場|上司|同僚|进度|進捗|需求|要件|延期|会議", "工作沟通"}, {"旅行|机场|出租|打车|点餐|日常|音乐|聊天|カフェ|レストラン|ホテル|予約|駅|休日|買い物", "日常生活"}} {
		...
		for _, line := range strings.Split(table, "\n") {
			...
			if len(cells) == 8 {
				...
				if cells[0] != "用户原话" && cells[0] != "---" {
					rows = append(rows, M{"original": cells[0], "japanese": cells[1], "chinese": cells[2], "note": "旧课次记载：" + cells[4] + "。这是当时保存的状态；未补造更详细的提示过程。"})
				}
			}
		}
	}
...
func (a *Archive) load() M {
	...
	for trial := 0; trial < 3; trial++ {
		...
		for _, v := range arr(state["concepts"]) {
			...
			for _, v := range terms {
				t := obj(v)
				if strings.Trim(strings.ToLower(str(t["japanese"])), " .?!") == strings.Trim(strings.ToLower(str(c["term"])), " .?!") && relatedIDs[str(t["id"])] != nil {
					matching = t
					break
				}
			}
			...
			t := M{"id": c["id"], "concept_id": c["id"], "japanese": c["term"], "chinese": c["meaning"], "kind": "知识点", "original": quote, "mastery": m, "state_label": c["level_label"], "source_session": source["id"], "source_title": source["title"], "date": source["date"], "updated": c["last_date"], "note": obj(events[len(events)-1])["note"], "next_review": next, "book": source["book"], "sources": sources, "seen_in_sessions": sids}
...
func (a *Archive) query(path string, args M) M {
	...
	case "/api/review":
		...
		return merge(meta, omit(job, "source", "worker_pid"), M{"thread_id": thread, "voice_id": voice, "created_epoch": get(job, "created_epoch", get(job, "started_epoch", 0)), "title": textOr(saved["title"], textOr(job["title"], "本次日语练习")), "practice_time": practiceTime, "session_id": saved["id"]})
	case "/api/reviews":
