...
func assertIdle(root string) {
	if exists(filepath.Join(root, "Live", "companion.sqlite3")) {
		...
		for _, v := range query(l.db, "SELECT * FROM runs") {
			s := stateRow(obj(v))
			require(s["desired"] == "stopped" || terminal(s["status"]), "日语练习或字幕排空尚未结束，请稍后切换目录。")
		}
	}
...
func backupBytes(root, project string, includeLive bool) ([]byte, M) {
	...
	withLock(filepath.Join(root, ".write.lock"), func() {
		...
		if includeLive && exists(filepath.Join(root, "Live", "companion.sqlite3")) {
			dir, e := os.MkdirTemp("", "japanese-coach-snapshot-")
			must(e)
	...
	manifest := M{"format": "japanese-speaking-coach-backup", "version": 1, "created_at": now(), "counts": counts, "includes_live": payload["data/Live/companion.sqlite3"] != nil, "includes_project_page": payload["context/project.md"] != nil, "files": files}
...
func inspectBackup(blob []byte) M {
	...
	require(manifest["format"] == "japanese-speaking-coach-backup" && integer(manifest["version"]) == 1, "不是受支持的完整学习备份。")
...
func restoreBackup(blob []byte, destination string, activate bool) M {
	...
	tmp, e := os.MkdirTemp(filepath.Dir(target), ".japanese-restore-")
	must(e)
...
func (s *Server) previewStorage(body M) M {
	...
	case "move", "restore":
		...
		if p.action == "move" {
			...
			temp, e := os.MkdirTemp("", "japanese-backup-check-")
			must(e)
