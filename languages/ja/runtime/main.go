...
func runCLI(args []string) any {
	...
	if str(o["vault"]) != "" {
		...
		w = M{"data_root": filepath.Join(base, "Japanese-Speaking"), "project_page": filepath.Join(base, "PRJ-JAPANESE-SPEAKING.md"), "mode": "vault", "config_path": configPath()}
	}
	...
	if cmd == "serve" {
		...
		defer cancel()
		serve(ctx, root, clampInt(o["port"], 8898, 0, 65535), str(o["root"]) == "")
		return nil
	}
	if cmd == "service" {
		action := "status"
		...
		case "status":
			return checkService(textOr(o["service-url"], "http://127.0.0.1:8898"), root, false)
		}
