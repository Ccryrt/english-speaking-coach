...
(async()=>{
  ...
  assert.equal(durable.elements.get('#review-notice').hidden,true,'Live view uses one compact review link');
  assert.match(durable.elements.get('#live-review').textContent,/本次复盘/);
  assert.match(durable.elements.get('#live-review').href,/thread=current&voice=voice/);
  assert.match(durable.elements.get('#live-review').title,/关闭 Voice/);
  ...
  vm.runInContext('paintReviewNotice()',durable.context);
  assert.equal(durable.elements.get('#review-notice').hidden,true);
  assert.match(durable.elements.get('#live-review').href,/thread=current&voice=voice/);
  durable.location.hash='#sessions/'+lesson.id;
